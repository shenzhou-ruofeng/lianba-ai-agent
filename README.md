...
>
> ⭐️ 加入项目系列学习：[加入编程导航](https://github.com/shenzhou-ruofeng/vip)
...
该脚本会依次启动：
1. MCP 图片搜索服务（端口 8127）
2. faster-whisper 语音识别服务（端口 10095）
3. Spring Boot 后端 API（端口 8123）
4. Vite 前端开发服务器（端口 3000）

启动成功后访问：**http://localhost:3000**

### 方式二：分步手动启动

#### 1. 启动 MCP 服务（必须先于后端启动）
...
1. **JDK 21**：项目使用 Java 21
2. **Maven 3.8+**：用于构建和管理项目依赖
3. **Node.js 18+**：前端开发需要
4. **Python 3.10+**：语音识别服务需要
5. **PostgreSQL 15+**：数据库和向量存储（已配置 pgvector 扩展）
6. **Git**：版本控制工具
...
文字版：https://github.com/shenzhou-ruofeng/course/1907378983347892226
